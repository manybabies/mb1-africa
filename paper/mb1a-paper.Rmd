...
affiliation:
  ...
  - id: "12"
    institution: "Humboldt-Universität, Berlin, Germany and University of Ghana, Legon, Ghana"
  - id: "13"
  ...
  - id: "21"
    institution: "Sapienza University, Italy & Kisii University, Kenya"
...
header-includes:
  ...
  - |
    ...
    \preto{\maketitle}{
      ...
      \begin{itemize}
        \item This study demonstrates a reliable infant-directed speech preference among African infants aged 3–15 months using a large-scale, multisite experimental design.
        \item Our findings showed no significant difference in IDS preference magnitude between African infants and method-matched samples from prior North American and European MB1 studies.
        \item Our study provides evidence for the cross-cultural generalizability of IDS preference while identifying practical challenges of conducting international multisite developmental research.
      \end{itemize}
      \newpage
      \setcounter{page}{1}
    }
...
nocite: |
  @fernald1989, @trainor2002pitch,@bryant2007recognizing,@legare2019development,@manybabies2020quantifying,@baumgartner2023build
...
Adults often speak to infants differently than to other adults, using a speech register known as infant-directed speech (IDS). Infant-directed speech tends to have exaggerated prosodic characteristics, including higher pitch, greater pitch variation, longer pauses, simplified grammatical structure, and shorter and slower utterances as compared to adult-directed speech (ADS; e.g. Fernald et al., 1989; Trainor & Desjardins, 2002). Even very young infants from a variety of language backgrounds have a preference for listening to IDS over ADS [ @cooper1994;@cooper1997;@fernald1985;@hayashi2001;@kitamura2009;@newman2006;@pegg1992;@santesso2007frontal;@singh2002;@werker1989infant]. Infants’ preference for IDS over ADS has also been demonstrated in a meta-analysis; across 34 studies, IDS preference had a fairly large average effect size with a value of Cohen’s d 0.72 [@dunst2012preference].

Why do infants prefer IDS? Perhaps IDS is intrinsically salient to infants because of its perceptual characteristics (e.g., higher pitch, greater pitch variability). Or perhaps, as infants are exposed to IDS, familiarity leads to preference. These explanations have different developmental predictions: while the intrinsic view would suggest an early preference [e.g.,@cooper1990], the exposure account would predict developmental increases in preference. Further, these explanations are not mutually exclusive: infants’ early preference for IDS may motivate their parents to use more IDS, which in turn could lead infants to show a stronger IDS preference. 
Regardless of its origins, infants’ preference for IDS may benefit their early language development. For example, in experimental studies, infants can segment words better in fluent speech produced in IDS than ADS [@thiessen2005infant], show better recognition of words introduced in IDS after a 24-hour delay [@singh2009influences], and more successfully learn words from IDS than ADS [@grafestes2013].

Further evidence comes from correlational studies, which have found that the amount of IDS in the language environment is positively related to children’s language outcomes, such as vocabulary size [e.g., @ramirez2014; @shneidman2013; @shneidman2012language; but cf., @casillas2020;@casillas2021,who found similar timing of language development milestones even in a population that hears very limited IDS]. Together, this work suggests that infants’ preference for IDS over ADS may support their language development, which explains why infants’ IDS preference continues to be an important topic in the literature on early childhood. 

However, it is important to note that almost all prior studies, including the meta-analysis by @dunst2012preference have included mainly infants learning English in Western, educated, industrialized, rich, and democratic (WEIRD) societies [@henrich2010weirdest], with only a few studies extended to non-Western infant populations learning languages other than English [@hayashi2001;@werker1994cross]. As such, there is a large sampling bias in the existing data about infants’ preference for IDS, as in many other research topics in developmental psychology [see @nielsen2017]. This sampling bias is a problem for generalizing findings about infants’ IDS preference to infants growing up in different cultures and learning different languages. In light of this generalizability issue – as well as the recent replication crisis in psychology [e.g., @openscience2015] – infant researchers have begun to collaborate on large-scale, multi-site studies to replicate key developmental findings [@frank2017]. 
...
```{r fig.width=8, figh.height = 8, fig.pos="t!", fig.cap="Map shows the location of the eight participating labs."}
...
```{r}
# exclude an unlabeled aggregation-artifact row (no lab) so the total tested = 274
total_tested_n <- sum(tested_demog$tested_participant_n[!is.na(tested_demog$lab)])
...
proportion_excluded_participants <- round(unusable_participants / total_tested_n, 4)
```
...
South Africa & 31 & 9.00 (3.71) & F: 16; M: 14; Unknown: 1 & Monolingual: 23; Other: 3; Unknown: 5 \\
...
```{r fig.pos="!t", fig.cap="Histograms of the age distribution in each lab. X-axis represents the age in months."}
...
d %>% 
  distinct(lab_clean, subid, age_months) %>% 
...
# mean age computed per infant (one row per subject), reported in months to match Table 2
mean_age_months <- round(joint_data %>% distinct(lab, subid, age_months) %>% summarise(mean_age = mean(age_months)) %>% pull(), 1)
min_age_months <- round(joint_data %>% summarise(min_age = min(age_months)) %>% pull(), 1)
max_age_months <- round(joint_data %>% summarise(max_age = max(age_months)) %>% pull(), 1)
...
* Infant Type: a dummy coded variable with two levels, representing infants living in Africa in our current study (coded as 1) and infants living in Europe, Australia and Asia who were not hearing North American English, with data from MB1(B) (coded as 0). 
...
```{r fig.pos="t!", fig.cap="By lab results. Each dot represents one trial of looking time. X-axis represents the trial number. Y-axis represents the log looking time in seconds. Red represents IDS trials. Blue represents ADS trials."}
d <- read_csv(here("processed_data/joint_data.csv")) %>% 
  filter(looking_time_s >= 2) %>% 
  mutate(trial_num_centered = scale(trial_num, scale = FALSE)) %>% 
  mutate(
...
\toprule
Term & Estimate [95\% CI] & std.error & t & p\\
\midrule
(Intercept) & 1.89 [1.66, 2.12] & 0.09 & 21.34 & < .01\\
Trial Type & 0.06 [0.02, 0.10] & 0.02 & 2.76 & 0.01\\
Trial Number & -0.03 [-0.04, -0.02] & 0.00 & -9.72 & < .01\\
Age & -0.09 [-0.14, -0.04] & 0.03 & -3.40 & < .01\\
Trial Type * Trial Number & 0.00 [-0.01, 0.01]  & 0.00 & -0.22 & 0.83\\
\addlinespace
Trial Number * Age & 0.00 [0.00, 0.01] & 0.00 & 1.60 & 0.11\\
Trial Type * Age & 0.02 [-0.02, 0.06] & 0.02 & 0.94 & 0.35\\
\bottomrule
\end{tabular}
\end{table}


The model revealed a significant main effect of trial type, such that infants looked longer at IDS trials than ADS trials (Figure 3, Figure 4; Table 3;  *$\beta$* = `r filter(q1_df, term == "trial_typeIDS")$estimate` [`r filter(q1_df, term == "trial_typeIDS")$conf.low`, `r filter(q1_df, term == "trial_typeIDS")$conf.high`]; *SE* = `r filter(q1_df, term == "trial_typeIDS")$std.error`; *t* = `r filter(q1_df, term == "trial_typeIDS")$statistic`; *p* = `r filter(q1_df, term == "trial_typeIDS")$p.value`). There was also a significant negative effect of trial number, indicating that looking times decreased over the course of the session (*$\beta$* = `r filter(q1_df, term == "trial_num_centered")$estimate` [`r filter(q1_df, term == "trial_num_centered")$conf.low`, `r filter(q1_df, term == "trial_num_centered")$conf.high`]; *SE* = `r filter(q1_df, term == "trial_num_centered")$std.error`; *t* = `r filter(q1_df, term == "trial_num_centered")$statistic`; *p* < 0.01). Age in months was also a significant predictor, with older infants showing shorter looking times overall (*$\beta$* = `r filter(q1_df, term == "age_months_scaled")$estimate` [`r filter(q1_df, term == "age_months_scaled")$conf.low`, `r filter(q1_df, term == "age_months_scaled")$conf.high`]; *SE* = `r filter(q1_df, term == "age_months_scaled")$std.error`; *t* = `r filter(q1_df, term == "age_months_scaled")$statistic`; *p* < 0.01). None of the interaction terms reached statistical significance, including the interaction between trial type and age, suggesting that the magnitude of IDS preference did not change reliably with age.


```{r fig.pos="!t",fig.cap="Percent increase in infants' looking time to infant-directed speech (IDS) relative to adult-directed speech (ADS) across six African sites and the meta-analytic estimate. While the meta-analytic estimate (red point) reflects the fixed-effect coefficient from the preregistered model, individual site estimates (black points) were obtained by fitting site-specific mixed-effects models predicting looking time using the same specification without the by-site random effect. Percent changes were computed from the fixed effect estimate, and horizontal lines represent 95\\% confidence intervals. Positive values indicate longer looking to IDS."}
...
`log_lt ~ trial_type + trial_num + age_months + infant_type + language_background + trial_type * trial_num + age_months * trial_num + age_months * trial_type + trial_type * infant_type + trial_num * infant_type + trial_type * language_background + (trial_type * trial_num | subid) + (trial_type | lab)`
...
After pruning for non-convergence, our final model specification was: `log_lt ~ trial_type + trial_num + age_months + infant_type + language_background + trial_type * trial_num + age_months * trial_num + age_months * trial_type + trial_type * infant_type + trial_num * infant_type + trial_type * language_background + (1 | subid) + (1 | lab)`. The fixed effect estimate corresponding to our research question is the `trial_type * infant_type`, which captures differences in measured IDS preference between the current data and data from MB1/MB1B in units of log seconds of looking time. 
...
\caption{Model estimates for Research Questions 3. The baseline for trial type is adult-directed speech (ADS), trial number is mean-centered, age is measured in months and standardized, the baseline for infant type is infants from MB1/MB1B (non-NAE sample), and the baseline for language background is monolingual infants.}
\centering
\begin{tabular}[t]{lcccc}
\toprule
Term & Estimate [95\% CI] & SE & t & p\\
\midrule
(Intercept) & 1.74 [1.62, 1.87] & 0.06 & 29.57 & < .01\\
Trial Type & 0.06 [0.04, 0.09] & 0.01 & 5.13 & < .01\\
Trial Number & -0.05 [-0.05, -0.05] & 0.00 & -31.08 & < .01\\
Age & -0.12 [-0.15, -0.09] & 0.01 & -8.26 & < .01\\
Infant Type & 0.13 [-0.09, 0.36] & 0.11 & 1.26 & 0.23\\
\addlinespace
Language Background (Bilingual) & 0.02 [-0.07, 0.12] & 0.05 & 0.42 & 0.68\\
Language Background (Other) & 0.02 [-0.10, 0.14] & 0.06 & 0.35 & 0.73\\
Trial Type * Trial Number & 0.00 [-0.01, 0.00] & 0.00 & -0.79 & 0.43\\
Trial Number * Age & 0.00 [-0.01, 0.00] & 0.00 & -3.79 & < .01\\
Trial Type * Age & 0.02 [0.00, 0.04] & 0.01 & 1.98 & 0.05\\
\addlinespace
Trial Type * Infant Type & 0.00 [-0.06, 0.06] & 0.03 & 0.03 & 0.97\\
Trial Number * Infant Type & 0.02 [0.01, 0.03] & 0.00 & 6.64 & < .01\\
Trial Type * Language Background (Bilingual) & -0.04 [-0.11, 0.02] & 0.03 & -1.46 & 0.14\\
Trial Type * Language Background (Other) & -0.04 [-0.11, 0.04] & 0.04 & -0.94 & 0.35\\
\bottomrule
\end{tabular}
\end{table}

The model revealed no significant difference in IDS preference between infants tested in our sample and those tested in MB1/MB1B, as indicated by the non-significant interaction between trial type and infant type (Table 4; *$\beta$* = `r filter(q3_df, term == "trial_typeIDS:infant_ID")$estimate` [`r filter(q3_df, term == "trial_typeIDS:infant_ID")$conf.low`, `r filter(q3_df, term == "trial_typeIDS:infant_ID")$conf.high`]; *SE* = `r filter(q3_df, term == "trial_typeIDS:infant_ID")$std.error`; *t* = `r filter(q3_df, term == "trial_typeIDS:infant_ID")$statistic`; *p* = `r filter(q3_df, term == "trial_typeIDS:infant_ID")$p.value`). However, the simple effect of trial type was significant for the monolingual reference group (*$\beta$* = `r filter(q3_df, term == "trial_typeIDS")$estimate` [`r filter(q3_df, term == "trial_typeIDS")$conf.low`, `r filter(q3_df, term == "trial_typeIDS")$conf.high`]; *SE* = `r filter(q3_df, term == "trial_typeIDS")$std.error`; *t* = `r filter(q3_df, term == "trial_typeIDS")$statistic`; *p* < .001), and the non-significant trial type × infant type interaction indicates a comparable preference in our African sample.  

Consistent with expectations, looking times decreased over the course of the session (*$\beta$* = `r filter(q3_df, term == "trial_num_centered")$estimate` [`r filter(q3_df, term == "trial_num_centered")$conf.low`, `r filter(q3_df, term == "trial_num_centered")$conf.high`]; *SE* = `r filter(q3_df, term == "trial_num_centered")$std.error`; *t* = `r filter(q3_df, term == "trial_num_centered")$statistic`; *p* < 0.01), and older infants looked for less time overall (*$\beta$* = `r filter(q3_df, term == "age_months_scaled")$estimate` [`r filter(q3_df, term == "age_months_scaled")$conf.low`, `r filter(q3_df, term == "age_months_scaled")$conf.high`]; *SE* = `r filter(q3_df, term == "age_months_scaled")$std.error`; *t* = `r filter(q3_df, term == "age_months_scaled")$statistic`; p < .001). We also found that older infants habituated more quickly, as indicated by a significant negative interaction between age and trial number (*$\beta$* = `r filter(q3_df, term == "trial_num_centered:age_months_scaled")$estimate` [`r filter(q3_df, term == "trial_num_centered:age_months_scaled")$conf.low`, `r filter(q3_df, term == "trial_num_centered:age_months_scaled")$conf.high`]; *SE* = `r filter(q3_df, term == "trial_num_centered:age_months_scaled")$std.error`; *t* = `r filter(q3_df, term == "trial_num_centered:age_months_scaled")$statistic`; p < .001). The interaction between trial number and infant type was also significant (*$\beta$* = `r filter(q3_df, term == "trial_num_centered:infant_ID")$estimate` [`r filter(q3_df, term == "trial_num_centered:infant_ID")$conf.low`, `r filter(q3_df, term == "trial_num_centered:infant_ID")$conf.high`]; *SE* = `r filter(q3_df, term == "trial_num_centered:infant_ID")$std.error`; *t* = `r filter(q3_df, term == "trial_num_centered:infant_ID")$statistic`; p < .001), indicating that looking times declined more slowly across trials for infants in our sample compared to those in MB1/MB1B. No other interactions reached significance.



## Exploratory Analyses

SES. Previous research in North America [e.g., @hart1995;@hoff2006social;@weisleder2013talking] has shown that the quantity and quality of child-directed speech vary across families with different SES backgrounds. These differences in language input may drive differences in infants’ preference for IDS. Thus, we explored how SES affects infants’ preference for IDS. SES was measured by primary caregiver's formal education (number of years). We entered primary caregiver’s formal education (in years) as a predictor in the regression model specified for RQ1 and RQ2, along with its interaction with trial type. 
...
The interaction between trial type and primary caregiver education was not significant (*$\beta$* = `r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$estimate` [`r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$conf.low`, `r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$conf.high`]; *SE* = `r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$std.error`; *t* = `r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$statistic`; *p* = `r filter(e1_df, term == "trial_typeIDS:scaled_primary_caregiver_ed")$p.value`), not allowing rejection of the null hypothesis that SES does not moderate infants' preference for IDS. In other words, the magnitude of IDS preference was similar regardless of caregivers’ years of formal education. At the same time, we did observe a significant main effect of trial type, with infants looking longer at IDS than ADS trials overall (*$\beta$* = `r filter(e1_df, term == "trial_typeIDS")$estimate` [`r filter(e1_df, term == "trial_typeIDS")$conf.low`, `r filter(e1_df, term == "trial_typeIDS")$conf.high`]; *SE* = `r filter(e1_df, term == "trial_typeIDS")$std.error`; *t* = `r filter(e1_df, term == "trial_typeIDS")$statistic`; *p* = `r filter(e1_df, term == "trial_typeIDS")$p.value`). Looking times decreased significantly across trials (*$\beta$* = `r filter(e1_df, term == "trial_num_centered")$estimate` [`r filter(e1_df, term == "trial_num_centered")$conf.low`, `r filter(e1_df, term == "trial_num_centered")$conf.high`]; *SE* = `r filter(e1_df, term == "trial_num_centered")$std.error`; *t* = `r filter(e1_df, term == "trial_num_centered")$statistic`; *p* < 0.01), and older infants looked for less time overall (*$\beta$* = `r filter(e1_df, term == "age_months_scaled")$estimate` [`r filter(e1_df, term == "age_months_scaled")$conf.low`, `r filter(e1_df, term == "age_months_scaled")$conf.high`]; *SE* = `r filter(e1_df, term == "age_months_scaled")$std.error`; *t* = `r filter(e1_df, term == "age_months_scaled")$statistic`; *p* < 0.01). No other interactions were significant. As a robustness check, we re-ran the model on the subset of infants with female primary caregivers (80.52% infants). The pattern of results was qualitatively unchanged.
...
Meta-analysis. For comparison with ManyBabies 1 and 1B, we computed standardized effect sizes for each lab using looking time, following the method used in ManyBabies Consortium (2020). We also confirmed that the effect size was similar when computing using log-transformed looking time. The resulting meta-analytic plot is shown in Appendix B: Figure B1. The meta-analytic effect size is `r es_df$effects`[`r es_df$effects.cil`, `r  es_df$effects.cih`] (log transformed: 0.14[-0.06, 0.34]), which is numerically smaller than the .35 [0.29, 0.42] reported in MB1 (Manybabies Consortium, 2020) and the .26 [0.09, 0.43] for bilingual infants reported in MB1B [@byers2021multilab]. But these estimates are not directly comparable due to the different method and age distribution in the current project [cf., @zettersten2024evidence].


# General Discussion

Infants' preference for IDS is both an important phenomenon for understanding language learning and a case study for infant methods more broadly. The MB1 study investigated variation in IDS preference across laboratories and across countries and found a small but reliable effect such that infants preferred IDS over ADS. Although there was substantial variation across labs, our analyses suggest that much of it reflected random sampling error, as the moderators we tested accounted for little of the between-lab variance. Nevertheless, the effect was moderated by infant age, language background, and experimental method. Though MB1 and its sister project MB1B investigated children from a wide variety of language backgrounds, no sites from Africa were included in this initial group of participating laboratories. The current study was designed to fill this gap.
...
In addition to logistical and training challenges, it is important to acknowledge the broader structural differences between the participating laboratories in this project and the Western laboratories that contributed to MB1 and MB1B. Most Western labs had access to established infrastructure for infant research, such as participant databases, mailing lists, as well as stable funding and institutional traditions of developmental experimentation. In contrast, many of the African sites in our study were building these capacities for the first time. Recruiting families without existing participant pools required extensive community engagement and word-of-mouth networks. The success of this effort therefore reflects not only careful coordination and training, but also the commitment and creativity of local researchers who developed sustainable pathways for infant recruitment and testing in settings with limited prior infrastructure.

These efforts resonate with broader calls to diversify and globalize infant research. Recent commentaries have highlighted that developmental science continues to draw disproportionately from Western contexts, and have urged the field to address the structural barriers that prevent broader participation [@singh2023diversity; @kidd2022diverse]. Equitable global collaboration requires attention not only to sampling diversity but also to the power dynamics and infrastructural inequities that shape how research is conducted and credited. Encouragingly, recent initiatives such as the /L+/ Global School on Language Acquisition [@aravena2024towards], co-led by several members of the MB1-Africa team, demonstrate how capacity building can foster sustainable participation from underrepresented regions. 

The current study has a number of further scientific limitations, including some shared with prior studies including MB1 and MB1B. First, although we attempted to estimate IDS preference, we did so using a specific set of speech stimuli and a specific paradigm. It is likely that the stimuli used here are less extreme than many used in prior studies, and further, they are produced in North American English, making them linguistically unfamiliar to one degree or another to all of the infants in our study. Followups using native language stimuli are needed to measure the importance of this choice to the IDS preference (i.e., MB1N, an in-progress follow up study to MB1 examining native language IDS preferences). Second, although we invited broad participation, our samples are convenience samples in at least two ways: both of the sites who participated and the infants who participated at each site. Thus our effect size estimates cannot be treated as population effects but rather "proof of concept" that an IDS preference can be observed in African infants across a diverse set of sites. Finally, although we did not observe major demographic variation, we caution against over-interpretation of any demographic differences in IDS preference given that IDS preference has not been shown to be individually predictive of any later outcomes [@soderstrom2024testing].

In sum, our study offers a case study of "big team science" [@coles2022build] carried out via a collaboration between African researchers and the ManyBabies Consortium. Although it faced a variety of logistical challenges (many shared with other grass roots efforts; Baumgartner et al., 2023), it nevertheless yields important evidence on generalizability, of a key phenomenon in early language learning, of looking time methods, and finally of a broad-based collaborative model for studying infant development. 
