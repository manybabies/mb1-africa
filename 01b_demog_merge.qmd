...
south_africa <- read_csv(here("processed_data/participants_cleaned/Ndhambi - South Africa - anjie_coded.csv"))
...
south_africa_clean <- south_africa %>% 
  ...
  rename(gender = sex) %>% 
   select(subid, lab, gender, age_months, language_background, hearing_prob, dev_disorder)
...
cleaned_trials_data <- trials_data %>% 
  ...
  filter(hearing_prob == "NO" | is.na(hearing_prob) | hearing_prob == "no") %>% 
  filter(dev_disorder == "NO" | dev_disorder == "NA" | is.na(dev_disorder) | dev_disorder == "no" ) %>% select(lab, subid, trial_num, trial_type, looking_time_s, stimulus, order, looking_time_diff, total_center, enabled_diff)
```
...
final_sample_d <- full_participnat_info %>% 
  filter(subid %in% joint_data$subid)
...
language_per_lab <- final_sample_d %>% 
  ...
  mutate(language_background = case_when(
    language_background == "monolinguals" ~ "monolingual", 
    language_background == "monolingual" ~ "monolingual", 
    is.na(language_background) ~ "lang_unknown", 
    TRUE ~language_background
  )) %>% 
  pivot_wider(names_from = language_background, values_from = language_background_n)
